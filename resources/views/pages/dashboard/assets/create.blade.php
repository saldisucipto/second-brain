...
<div class="space-y-6">
  ...
  <div class="rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-white/[0.03]">
    <a href="{{ route('assets.index') }}" class="inline-flex items-center gap-2 text-sm text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-300 mb-4">
    ...
    <form action="{{ route('assets.store') }}" method="POST" class="space-y-6">
      ...
      <div>
        ...
        <textarea name="note" rows="3" placeholder="Informasi tambahan tentang asset..."
          class="w-full rounded-lg border border-gray-300 bg-white px-4 py-2.5 text-sm text-gray-800 focus:border-brand-500 focus:outline-none dark:border-gray-700 dark:bg-gray-900 dark:text-white/90">{{ old('note') }}</textarea>
      </div>
